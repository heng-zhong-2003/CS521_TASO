...
class PatternToOnnxExpr:
    ...
    def get_dim_runtime_value_ast(self, op: Operator, dim: int) -> AST:
        ...
        index_ast = ast.Call(
            ...
            args=[],
            keywords=[ast.keyword(arg='value_int', value=ast.Constant(value=dim))]
        )
    ...
    def split_to_onnx_pattern(self, gop: SplitOperator):
        ...
        def make_unsqueeze(expr: ast.expr) -> ast.Call:
            return ast.Call(
                func=ast.Attribute(
                    value=ast.Name(id=self.onnx_pattern_op_name, ctx=ast.Load()),
                    attr='Unsqueeze',
        ...
        split_pos_unsq_ast = make_unsqueeze(split_pos_ast)
        second_len_unsq_ast = make_unsqueeze(second_len_ast)
        ...
        split_ast = ast.Call(
            ...
            args=[inp_onnx, splits_tensor_ast],
            keywords=[
    ...
    def pattern_to_onnx_pattern(self, pattern_graph: Graph) -> tuple[ast.AST, list[str]]:
        output_exprs: list[ast.expr] = []
        for out in pattern_graph.get_outputs():
            if isinstance(out, SplitOperator):
                split_ast = self.op_to_onnx_expr(out)
                output_exprs.append(
                    self.get_indexing_split_result_ast(split_ast, 0)
                )
                output_exprs.append(
                    self.get_indexing_split_result_ast(split_ast, 1)
                )
            else:
                output_exprs.append(self.op_to_onnx_expr(out))

        if len(output_exprs) == 1:
            outputs_ast: ast.AST = output_exprs[0]
        else:
            outputs_ast = ast.Tuple(elts=output_exprs, ctx=ast.Load())
